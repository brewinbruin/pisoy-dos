...
function createRoom(hostName, hostSocketId) {
  ...
  rooms[code] = {
    ...
    firstPlayDone: false,
    openingCard: null,
    playHistory: [],   // last 3 played combos: [{ name, combo, ts }, ...]  newest-first
  };
...
function joinRoom(code, playerName, socketId) {
  ...
  if (room.players.find(p => p.name.toLowerCase() === playerName.toLowerCase())) {
    return { error: 'That name is already taken — pick a different name' };
  }
...
function rejoinRoom(code, playerName, newSocketId) {
  ...
  if (!existing) {
    // Not a known player — treat as new join if lobby or revealing
    if (room.phase === 'lobby' || room.phase === 'revealing') return joinRoom(code, playerName, newSocketId);
...
function playCards(code, socketId, cardCodes) {
  ...
  // Record to play history (newest first, max 3)
  const histEntry = { name: playerName(room, socketId), combo: { type: combo.type, cards: combo.cards }, ts: Date.now() };
  room.playHistory = [histEntry, ...room.playHistory].slice(0, 3);
