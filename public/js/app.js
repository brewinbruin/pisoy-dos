...
function renderLobby(lobby) {
  ...
  const me = lobby.players.find(p => p.id === mySocketId);
...
function renderGame(state, myHand) {
  ...
  if (state.currentCombo) {
    ...
    $('played-label').textContent = isMyTurn && state.controlPlayer === mySocketId
  ...
  let statusText = '';
  if (isMyTurn) {
    statusText = state.controlPlayer === mySocketId ? '🎯 Your turn — you have control' : '🎯 Your turn!';
  } else {
...
function updateButtons(state, isMyTurn) {
  ...
  const inControl = state?.controlPlayer === mySocketId;
